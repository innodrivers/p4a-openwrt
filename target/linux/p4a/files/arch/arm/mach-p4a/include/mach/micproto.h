...
static inline u32 MREQB_POP_ARG(struct mreqb *mreqb)
{
	if (mreqb->argc > 0)
...
static inline u32 MREQB_GET_ARG(struct mreqb *mreqb, int index)
{
...
extern int __init micproto_init(void);

extern struct mreqb *mreqb_alloc(int extra_data_size);
